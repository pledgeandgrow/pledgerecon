...
pub fn builtin_patterns() -> Vec<SecretPattern> {
    vec![
        ...
        SecretPattern {
            ...
            severity: SecretSeverity::Critical,
            regex: r"gh[pousr]_[A-Za-z0-9]{36}".into(),
            min_entropy: 3.5,
        },
        SecretPattern {
            ...
            secret_type: SecretType::GitHubToken,
            severity: SecretSeverity::Critical,
            regex: r"github_pat_[A-Za-z0-9_]{82}".into(),
            min_entropy: 3.5,
        },
        SecretPattern {
            ...
            regex: r"glpat-[A-Za-z0-9_-]{20}".into(),
            min_entropy: 3.0,
        },
        SecretPattern {
            ...
            severity: SecretSeverity::High,
            regex: r"xox[baprs]-[A-Za-z0-9-]{10,}".into(),
            min_entropy: 3.0,
        },
        SecretPattern {
        ...
        SecretPattern {
            ...
            secret_type: SecretType::GoogleOAuthClientSecret,
            severity: SecretSeverity::High,
            regex: r"GOCSPX-[A-Za-z0-9_-]{35}".into(),
            min_entropy: 3.0,
        },
        SecretPattern {
            name: "azure_storage".into(),
        ...
        SecretPattern {
            ...
            secret_type: SecretType::PrivateKeyPem,
            severity: SecretSeverity::Critical,
        ...
        SecretPattern {
            name: "twilio_key".into(),
            secret_type: SecretType::TwilioApiKey,
            severity: SecretSeverity::High,
            regex: r"SK[0-9a-fA-F]{32}".into(),
            min_entropy: 3.0,
        },
        SecretPattern {
            name: "sendgrid_key".into(),
            secret_type: SecretType::SendGridApiKey,
            severity: SecretSeverity::High,
            regex: r"SG\.[A-Za-z0-9_]{22}\.[A-Za-z0-9_]{43}".into(),
            min_entropy: 3.0,
        },
        SecretPattern {
            name: "mailgun_key".into(),
            secret_type: SecretType::MailgunApiKey,
            severity: SecretSeverity::High,
            regex: r"key-[0-9a-zA-Z]{32}".into(),
            min_entropy: 3.0,
        },
        SecretPattern {
            name: "generic_password".into(),
            secret_type: SecretType::GenericPassword,
            severity: SecretSeverity::Medium,
            regex: r#"(?i)(password|passwd|pwd)\s*[=:]\s*["']([^"'\s]{8,})["']"#.into(),
            min_entropy: 2.0,
        },
        SecretPattern {
            ...
            severity: SecretSeverity::Medium,
            regex: r#"(?i)(api[_-]?key|apikey)\s*[=:]\s*["']([A-Za-z0-9_\-]{20,})["']"#.into(),
            min_entropy: 3.0,
        },
        SecretPattern {
            name: "generic_token".into(),
            secret_type: SecretType::GenericToken,
            severity: SecretSeverity::Medium,
            regex: r#"(?i)(token|secret|auth)\s*[=:]\s*["']([A-Za-z0-9_\-]{20,})["']"#.into(),
            min_entropy: 3.0,
        },
    ]
}
...
fn scan_line(
    ...
    for cp in patterns {
        for m in cp.regex.find_iter(line) {
            ...
            if cp.pattern.min_entropy > 0.0 && shannon_entropy(&val) < cp.pattern.min_entropy {
                continue;
            }
            findings.push(SecretFinding {
                secret_type: cp.pattern.secret_type.clone(),
                severity: cp.pattern.severity.clone(),
                location: SecretLocation::SourceCode {
                    file: file.to_string(),
                    line: line_num,
                },
                fingerprint: fingerprint(&val),
                redacted_preview: redact(&val, 4, 4),
                raw_value: val,
                pattern_name: cp.pattern.name.clone(),
                verified: false,
            });
        }
    }
...
) -> Result<SecretScanResult, SecretError> {
            ...
            for (i, line) in content.lines().enumerate() {
                for cp in &compiled {
                    for m in cp.regex.find_iter(line) {
                        ...
                        if cp.pattern.min_entropy > 0.0
                            && shannon_entropy(&val) < cp.pattern.min_entropy
                        {
                            continue;
                        }
                        findings.push(SecretFinding {
                            secret_type: cp.pattern.secret_type.clone(),
                            severity: cp.pattern.severity.clone(),
                            location: SecretLocation::ContainerImage {
...
pub fn scan_iac_files(
    files: &[(IaCKind, String, String)],
) -> Result<SecretScanResult, SecretError> {
    let compiled = compile_patterns(&builtin_patterns())?;
    let mut findings = Vec::new();
        ...
        for m in token_re.find_iter(line) {
            ...
            if ent >= min_entropy {
                findings.push(SecretFinding {
                    ...
                    pattern_name: format!("entropy({:.2})", ent),
                    verified: false,
                });
            }
        }
...
fn verify_single(st: &SecretType, val: &str) -> (bool, String) {
    match st {
        SecretType::AwsAccessKey => {
            if val.len() == 20 && val.starts_with("AKIA") {
                (true, "Valid AWS key format".into())
            } else {
                (false, "Invalid format".into())
            }
        }
        SecretType::GitHubToken => {
            if val.starts_with("ghp_")
                || val.starts_with("gho_")
                || val.starts_with("ghu_")
                || val.starts_with("ghs_")
                || val.starts_with("ghr_")
                || val.starts_with("github_pat_")
            {
                (true, "Valid GitHub token format".into())
            } else {
                (false, "Invalid format".into())
            }
        }
        SecretType::PrivateKeyPem => {
            if val.contains("-----BEGIN") && val.contains("PRIVATE KEY-----") {
                (true, "Private key detected".into())
            } else {
                (false, "Invalid format".into())
            }
        }
                ...
                for cp in &compiled {
                    for m in cp.regex.find_iter(line) {
                        ...
                        findings.push(SecretFinding {
                            secret_type: cp.pattern.secret_type.clone(),
                            severity: cp.pattern.severity.clone(),
                            location: SecretLocation::GitHistory {
                                commit: commit.commit_sha.clone(),
                                file: file_path.clone(),
                                line: i + 1,
                            },
                            fingerprint: fingerprint(&val),
...
pub fn scan_env_files(files: &[(String, String)]) -> Result<SecretScanResult, SecretError> {
    ...
    let sensitive_re = Regex::new(
        r"(?i)(password|passwd|pwd|secret|token|api[_-]?key|apikey|private[_-]?key|access[_-]?key|client[_-]?secret|auth)",
    )?;
    let mut findings = Vec::new();
            ...
            if let Some(eq) = trimmed.find('=') {
                ...
                if sensitive_re.is_match(key) && !val.is_empty() {
                    findings.push(SecretFinding {
                        secret_type: SecretType::GenericPassword,
                        severity: SecretSeverity::High,
                        location: SecretLocation::EnvFile {
                            file: file_path.clone(),
                            line: i + 1,
                        },
                        fingerprint: fingerprint(val),
                        redacted_preview: redact(val, 2, 2),
                        raw_value: val.to_string(),
                        pattern_name: "env_sensitive_var".into(),
                        verified: false,
                    });
                    continue;
                }
    ...
    for manifest in manifests {
        for (i, line) in manifest.content.lines().enumerate() {
            let matches: Vec<(SecretType, String)> = match manifest.kind {
                ManifestKind::Npmrc => npm_auth_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericToken, c[1].trim().to_string())])
                    .unwrap_or_default(),
                ManifestKind::CargoConfig => cargo_token_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericToken, c[1].to_string())])
                    .unwrap_or_default(),
                ManifestKind::PipConf => pip_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericPassword, c[2].to_string())])
                    .unwrap_or_default(),
                ManifestKind::MavenSettings => maven_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericPassword, c[1].to_string())])
                    .unwrap_or_default(),
                ManifestKind::NugetConfig => nuget_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericApiKey, c[1].to_string())])
                    .unwrap_or_default(),
                ManifestKind::DockerConfig => docker_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericToken, c[1].to_string())])
                    .unwrap_or_default(),
                ManifestKind::GradleProperties => gradle_re
                    .captures(line)
                    .map(|c| vec![(SecretType::GenericPassword, c[2].trim().to_string())])
                    .unwrap_or_default(),
            ...
            for (st, val) in matches {
                findings.push(SecretFinding {
                    secret_type: st,
                    severity: SecretSeverity::High,
                    location: SecretLocation::Manifest {
                        file: manifest.file.clone(),
                        field: format!("line {}", i + 1),
                    },
                    fingerprint: fingerprint(&val),
                    redacted_preview: redact(&val, 4, 4),
                    raw_value: val,
                    pattern_name: format!("manifest_{:?}", manifest.kind).to_lowercase(),
                    verified: false,
                });
            }
            for cp in &compiled {
...
pub fn rotation_guidance_for_scan(result: &SecretScanResult) -> Vec<RotationGuidance> {
    ...
    result
        .findings
        .iter()
        .filter(|&f| seen.insert(f.secret_type.to_string()))
        .map(|f| rotation_guidance(&f.secret_type))
        .collect()
}
...
pub fn secret_scan_report(result: &SecretScanResult) -> String {
    ...
    for f in &sorted {
        let loc = match &f.location {
            ...
            SecretLocation::ContainerImage { layer, file, line } => {
                format!("{}:{}/{}:{}", layer, file, line, "")
            }
            SecretLocation::IaC { file, line, .. } => format!("{}:{}", file, line),
            SecretLocation::GitHistory { commit, file, line } => {
                format!("{}@{}:{}", commit, file, line)
            }
...
mod tests {
    ...
    fn test_scan_container_image() {
        ...
        assert!(
            result
                .findings
                .iter()
                .any(|f| matches!(f.location, SecretLocation::ContainerImage { .. }))
        );
    }
    ...
    fn test_scan_env_files() {
        ...
        assert!(
            result
                .findings
                .iter()
                .any(|f| matches!(f.location, SecretLocation::EnvFile { .. }))
        );
    }
    ...
    fn test_scan_manifests_npmrc() {
        ...
        assert!(
            result
                .findings
                .iter()
                .any(|f| f.secret_type == SecretType::GenericToken)
        );
    }
    ...
    fn test_scan_manifests_cargo_config() {
        let manifests = vec![ManifestToScan {
            file: ".cargo/config.toml".into(),
            kind: ManifestKind::CargoConfig,
    ...
    fn test_scan_manifests_maven_settings() {
        let manifests = vec![ManifestToScan {
            file: "settings.xml".into(),
            kind: ManifestKind::MavenSettings,
        ...
        assert!(
            result
                .findings
                .iter()
                .any(|f| f.secret_type == SecretType::GenericPassword)
        );
    }
    ...
    fn test_rotation_guidance_for_scan() {
        let result = SecretScanResult::from_findings(
            vec![
                make_finding(
                    SecretType::AwsAccessKey,
                    SecretSeverity::Critical,
